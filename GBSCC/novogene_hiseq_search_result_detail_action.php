...
if (!$res) {
    echo "<hr><p style=\"font-family:sans-serif;font-size:20px;color:red\"><br>Request $run failed to updated. Please contact Genomics Core support: siyunliu@um.edu.mo<br><br></p>" . mysql_error();
} else {
    ...
    if ($result_user[0]['main'] == y) {
        ...
        if ($search_result[$i]['Approval'] == "Confirmed") {
            ...
            if ($search_result[$i]['Form_file'] != "" && $search_result[$i]['Contract_file'] != "") {
                ...
                if ($tomail != "") {
                    ...
                    if (!$mail->Send()) {
                        echo "<p>Confirmation email failed. Please contact Genomics Core support: siyunliu@um.edu.mo<br><br></p>" . $mail->ErrorInfo;
                    } else {
        ...
        if ($search_result[$i]['Approval'] == "Rejected") {
                ...
                if ($tomail != "") {
                    ...
                    //send the message, check for errors
                    if (!$mail->Send()) {
                        echo "<p>Email failed to send to request submitter. Please contact Genomics Core support: siyunliu@um.edu.mo<br><br></p>" . $mail->ErrorInfo;
                    } else {
