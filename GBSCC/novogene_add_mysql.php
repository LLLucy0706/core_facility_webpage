...
if (is_uploaded_file($_FILES['Form_file']['tmp_name'])) {
    ...
    if ($okType) {
        ...
        if (move_uploaded_file($tmp_name, $Form_file)) {
            ...
            echo "<p><br>Sample information form (xlsx) upload failed. Please contact Genomics Core support: siyunliu@um.edu.mo<br><br></p>";
            exit;
        }
    } else {
...
if (is_uploaded_file($_FILES['Contract_file']['tmp_name'])) {
    ...
    if ($okType) {
        ...
        if (move_uploaded_file($tmp_name, $Contract_file)) {
            ...
            echo "<p><br>Quotation file (PDF) upload failed. Please contact Genomics Core support: siyunliu@um.edu.mo<br><br></p>";
            exit;
        }
    } else {
...
if($RNA_extraction != "" && $library_QC != "" && $sequencing != "" && $RNA6G != "" && $RNA12G != "" && $ChIP6G != "" && $ChIP12G != "") {
    ...
    if ($Form_file != "" && $Contract_file != "") {
        ...
        if ($total_cost_usd != "" && $total_cost != "" && $total_cost_usd != 0 && $total_cost != 0) {
            ...
            if (!$res) {
                echo "<br><hr><br><b><span style=\"font-family:sans-serif;font-size:20px;color:red\">Your request submission failed. Please contact Genomics Core support: siyunliu@um.edu.mo</span></b>";
            } else {
                ...
                if($tomail!=""){
                    ...
                    if(!$mail->Send()) {
                        echo "<br><br>Request email failed. Please contact Genomics Core support: siyunliu@um.edu.mo<br><br>" . $mail->ErrorInfo;
                    } else {
